...
describe("AgendaEventos", function () {
  ...
  it("Debería permitir crear un nuevo evento", async function () {
    ...
    expect(evento.nombre).to.equal("Reunión de equipo");
    expect(evento.descripcion).to.equal("Discusión sobre el proyecto");
    expect(evento.fecha).to.equal(1672531200);
  ...
  it("Debería permitir obtener un evento existente", async function () {
    ...
    const evento = await agendaEventos.getEvent(0);
    expect(evento.nombre).to.equal("Conferencia");
    expect(evento.descripcion).to.equal("Presentación importante");
    expect(evento.fecha).to.equal(1672704000);
  });
  ...
  it("Debería permitir modificar un evento existente", async function () {
    ...
    expect(eventoActualizado.nombre).to.equal("Evento nuevo");
    expect(eventoActualizado.descripcion).to.equal("Descripción nueva");
    expect(eventoActualizado.fecha).to.equal(1672876800);
  ...
  it("Debería manejar eventos de diferentes usuarios correctamente", async function () {
    ...
    await expect(agendaEventos.getEvent(0)).to.not.be.reverted; // El owner puede ver su evento
    await expect(agendaUser1.getEvent(0)).to.not.be.reverted; // El usuario 1 puede ver su evento
    await expect(agendaEventos.connect(user1).getEvent(0)).to.be.revertedWith("Evento no existe"); // El usuario 1 no puede ver el evento del owner con ID 0
  });
});